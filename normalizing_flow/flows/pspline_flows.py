...
class PNSF(nn.Module):
    ...
    def __init__(self, dim, K=5, B=3):
        ...
        self.K = K
        self.B = B
        self.p1 = nn.Parameter(0.1 * torch.randn((3 * K - 1), device=constants.DEVICE), requires_grad=True)
        self.p2 = nn.Parameter(0.1 * torch.randn((3 * K - 1), device=constants.DEVICE), requires_grad=True)
