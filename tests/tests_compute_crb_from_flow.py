...
class FlowToCRBTest(unittest.TestCase):
    ...
    def model_init(self, in_model, in_dim, theta_value):
        ...
        return nf.NormalizingFlowModel(prior, [in_model.get_optimal_model()]), theta, crb
    ...
    def test_backward_flow_multiplication(self):
        dim = 2
        lm = MultiplicationModel(dim, 0.3, 10.0)
        status = self.run_test_backward(lm, dim, 2)
        self.assertTrue(status)

    def test_flow_multiplication(self):
        dim = 4
        lm = MultiplicationModel(dim, 0.3, 10.0)
        status = self.run_test_dual(lm, dim, 0.1)
        self.assertTrue(status)

    def test_backward_flow_multiplication_simple(self):
        dim = 2
        lm = MultiplicationSimple(dim, 0.3, 10.0)
        status = self.run_test_backward(lm, dim, 2)
        self.assertTrue(status)

    def test_flow_multiplication_simple(self):
        dim = 4
        lm = MultiplicationSimple(dim, 0.3, 10.0)
        status = self.run_test_dual(lm, dim, 0.1)
        self.assertTrue(status)
