...
def adaptive_sampling_gfim(model, in_theta_tensor, batch_size=128, eps=0.01, p_min=0.01,
    ...
    u = -np.log(p_min)
    status = True
    iteration_step = int(iter_size * n_max / batch_size)
    fim_collector = None
    with tqdm(total=n_max) as pbar:
        ...
        while status:
            ...
            e_norm = fim_collector.calculate_score_mean_norm()  # L2 norm of the score vector
            # e_norm = fim_collector.calculate_score_max_norm()  # L2 norm of the score vector
